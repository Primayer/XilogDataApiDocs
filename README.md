...
The Xilog API provides access to the raw data recorded by the Xilog logger. To use the API third parties must provide an authorization token with each request. At the moment this API only supports the 4G variant of Xilog. The authorization token can be found <a href=https://atriumiot.com/accountmanagement> here</a>
